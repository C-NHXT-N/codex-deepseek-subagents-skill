...
def summarize_usage(rows):
    ...
    for row in usage_rows:
        ...
        entry = by_model.setdefault(label, {
            "model_label": label,
            "requests": 0,
            "prompt_tokens": 0,
            "completion_tokens": 0,
        ...
        entry["requests"] += 1
        for key in ("prompt_tokens", "completion_tokens", "reasoning_tokens", "prompt_cache_hit_tokens", "prompt_cache_miss_tokens", "total_tokens"):
            entry[key] += int(row.get(key) or 0)
    hit = sum(int(row.get("prompt_cache_hit_tokens") or 0) for row in usage_rows)
    miss = sum(int(row.get("prompt_cache_miss_tokens") or 0) for row in usage_rows)
    return {
        ...
        "cache_hit_ratio": (float(hit) / float(hit + miss)) if (hit + miss) else None,
...
def render_usage(summary):
    ...
    lines = [
        ...
        f"Cache hit tokens: {summary.get('prompt_cache_hit_tokens') or 0}",
        f"Cache miss tokens: {summary.get('prompt_cache_miss_tokens') or 0}",
        f"Cache hit ratio: {ratio:.0%}" if isinstance(ratio, (float, int)) else "Cache hit ratio: n/a",
    ]
