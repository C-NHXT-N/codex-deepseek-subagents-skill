...
def _require_patch_ids(args):
    if not args.task_id:
        raise RuntimeError("--task-id is required")
    if not args.patch_id and args.command != "list-patches":
        raise RuntimeError("--patch-id is required")


def command_list_patches(args):
    project_root = Path(args.project_root).resolve()
    state = load_state(project_root, port_override=args.port)
    if not args.task_id:
        raise RuntimeError("--task-id is required")
    patches = state.list_patches(args.task_id)
    if args.json:
        print_json({"task_id": args.task_id, "patches": patches})
        return
    for patch in patches:
        print(f"{patch['patch_id']}  {patch['status']}  files={len((patch.get('summary') or {}).get('files') or [])}")


def command_show_patch(args):
    project_root = Path(args.project_root).resolve()
    state = load_state(project_root, port_override=args.port)
    _require_patch_ids(args)
    patch = copy.deepcopy(state.find_pending_patch(args.task_id, args.patch_id))
    patch["patch_text"] = state._read_patch_body(args.patch_id)
    if args.json:
        print_json(patch)
        return
    print_json(patch)
...
def command_approve_patch(args):
    project_root = Path(args.project_root).resolve()
    state = load_state(project_root, port_override=args.port)
    _require_patch_ids(args)
    maybe_confirm(args.yes, required=True)
    patch = state.approve_patch(args.task_id, args.patch_id, {"approval_note": "approved from CLI"})
    if args.json:
        print_json(patch)
        return
    print(f"Approved patch {args.patch_id}")


def command_reject_patch(args):
    project_root = Path(args.project_root).resolve()
    state = load_state(project_root, port_override=args.port)
    _require_patch_ids(args)
    maybe_confirm(args.yes, required=True)
    patch = state.reject_patch(args.task_id, args.patch_id, {"rejection_note": "rejected from CLI"})
    if args.json:
        print_json(patch)
        return
    print(f"Rejected patch {args.patch_id}")
...
def command_apply_patch(args):
    ...
    _require_patch_ids(args)
    maybe_confirm(args.yes, required=True)
    task = state.find_task(args.task_id)
    if not task:
        raise RuntimeError(f"Unknown task: {args.task_id}")
    result = state.apply_approved_patch(task, args.patch_id)
    if args.json:
        print_json(result)
        return
    print_json(result)
...
def build_parser():
    ...
    parser.add_argument("command", choices=["doctor", "start-runtime", "stop-runtime", "test-runtime", "test-proxy", "delegate", "analyze", "usage", "tui", "list-patches", "show-patch", "approve-patch", "reject-patch", "apply-patch"])
    ...
    parser.add_argument("--max-tool-steps", type=int, default=scheduler.DEFAULT_MAX_TOOL_STEPS)
...
def main():
    ...
    if args.command == "list-patches":
        return command_list_patches(args)
    if args.command == "show-patch":
        return command_show_patch(args)
    if args.command == "approve-patch":
        return command_approve_patch(args)
    if args.command == "reject-patch":
        return command_reject_patch(args)
