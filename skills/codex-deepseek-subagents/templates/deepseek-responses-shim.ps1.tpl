...
try {
    while ($listener.IsListening) {
        ...
        try {
            ...
            $model = if ($payload.model) { [string]$payload.model } elseif ($env:DEEPSEEK_OPENAI_MODEL) { $env:DEEPSEEK_OPENAI_MODEL } else { "__MODEL_PS__" }
            $messages = Convert-ResponseInputToMessages -ResponseInput $payload.input
            if ($messages.Count -eq 0) {
                $messages = @(@{ role = "user"; content = "Respond with exactly: ok" })
            }

            $defaultEffort = if ($env:DEEPSEEK_THINKING_DEFAULT) { $env:DEEPSEEK_THINKING_DEFAULT } else { "__THINKING_DEFAULT_PS__" }
